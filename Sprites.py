...
class Enemy(p.sprite.Sprite):
    def __init__(self, Game, x, y):
        ...
        self.alert_img = Game.alerted
        self.alreadyalert = False
        self.counter = 0
    ...
    def update(self):
        ...
        apos = self.pos + ABOVE
        if player_dist.length() < DETECT_RADIUS:
            if self.counter == 0:
                E_alert(self.Game, apos)
            self.alreadyalert = True
            while self.alreadyalert:
                if track_player <= 90:
...
class E_alert(p.sprite.Sprite):
    def __init__(self, Game, pos):
        self.groups = Game.all_sprites , Game.alert
        p.sprite.Sprite.__init__(self,self.groups)
        self.Game = Game
        self.image = Game.alerted
        self.rect = self.image.get_rect()
        self.pos = vec(pos)
        self.rect.center = pos
        self.spawn_time = p.time.get_ticks()
...
class BIG(p.sprite.Sprite):
    ...
    def update(self):
        ...
        player_dist = self.Game.player.pos - self.pos
        if player_dist.length_squared() < B_DETECTION_RADIUS**2:
    ...
    def attack1(self):
        BBullet(self.Game, self.bpos, self.direction)
        
    def attack2(self):
        BBullet(self.Game, self.bpos, ONEZERO)
        BBullet(self.Game, self.bpos, ONEONE)
        BBullet(self.Game, self.bpos, ZEROONE)
        BBullet(self.Game, self.bpos, M_ONEONE)
        BBullet(self.Game, self.bpos, M_ONEZERO)
        BBullet(self.Game, self.bpos, M_ONE_ONE)
        BBullet(self.Game, self.bpos, ZERO_ONE)
        BBullet(self.Game, self.bpos, ONE_ONE)
